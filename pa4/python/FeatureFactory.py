...
#                features.append("case=Title")
        ...
        if position < len(words) - 2:
            if words[position+1] == '\'' and words[position+2] == 's':
                features.append('possesive')
        if currentWord.find('-') and currentWord.replace('-','').isalpha() > 0:
            features.append('alpha_with_dashes')

#        if len(currentWord)<3:
#            features.append("too_short_to_be_name")
